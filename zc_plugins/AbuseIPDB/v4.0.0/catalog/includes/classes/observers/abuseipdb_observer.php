...
if (!$ip_info->EOF && (time() - strtotime($ip_info->fields['timestamp'])) < $cache_time) {
    ...
    // Only flood-track if not already tracked
...
// Load country score minimum (only once)
...
if (ABUSEIPDB_FOREIGN_FLOOD_ENABLED == 'true' && $countryCode) {
    $default_country = ABUSEIPDB_DEFAULT_COUNTRY;
    if (strcasecmp($countryCode, $default_country) !== 0) {
        $resForeign = $db->Execute("SELECT count FROM " . TABLE_ABUSEIPDB_FLOOD . " WHERE prefix = '" . zen_db_input($countryCode) . "' AND prefix_type = 'country'");
        if (!$resForeign->EOF && $resForeign->fields['count'] >= (int)ABUSEIPDB_FOREIGN_FLOOD_THRESHOLD) {
            if ($abuseScore >= $country_min_score) {
                $flood_block = true;
            }
        }
...
// --- [End Flood Blocking] ---
    ...
    // IP not found or cache expired, make API call
    ...
    if (!$ip_info->EOF) {
        // Existing IP: update score and timestamp
        ...
        // New IP: insert with flood_tracked = 0
        $insert_query = "INSERT INTO " . TABLE_ABUSEIPDB_CACHE . " (ip, score, country_code, timestamp, flood_tracked) 
                         ...
                         ON DUPLICATE KEY UPDATE score = VALUES(score), country_code = VALUES(country_code), timestamp = VALUES(timestamp)";
        $db->Execute($insert_query);
    }
    ...
    // After inserting/updating, always flood-track
    updateFloodTracking($ip, $countryCode, $prefix2, $prefix3);
    $db->Execute("UPDATE " . TABLE_ABUSEIPDB_CACHE . " SET flood_tracked = 1 WHERE ip = '" . zen_db_input($ip) . "'");
    ...
    if ($flood_block) {
        $log_file_path_flood = ABUSEIPDB_LOG_FILE_PATH . $log_file_name;
        $log_message_flood = date('Y-m-d H:i:s') . ' IP address ' . $ip . ' blocked by Flood Protection.' . PHP_EOL;
        if ($enable_logging) {
            file_put_contents($log_file_path_flood, $log_message_flood, FILE_APPEND);
        }
        if ($redirect_option === 'page_not_found') {
